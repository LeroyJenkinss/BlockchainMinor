...
def load_files(key_file_name):
    loadfile = open('message_test.enc')
    file = open('samples_test.keys')
    for k in key:
        decrypt_private = decrypt(file, key[1])
        decrypt_public = decrypt(loadfile, key[1])
